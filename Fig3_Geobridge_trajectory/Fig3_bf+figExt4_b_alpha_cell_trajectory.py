...
def get_All_t(All_data, cluster_labels, init_target, reg=1e-3):
    ...
    KP = -FISTA_OT.fista_ot2(C2, mu, nu, lambda_reg=reg, max_iter=1000, tol=1e-6)[1]
    KP_max = KP.max()
    KP_min = KP.min()
...
pseudotime, All_t = get_All_t(All_data, cluster_labels, init_target, reg=2e-1)
    ...
    for retry in range(max_retry):
        try:
            with warnings.catch_warnings():
                # Raise RuntimeWarning as an exception
                warnings.filterwarnings(
                    "error",
                    category=RuntimeWarning
                )

                pseudotime_z, All_t_z = get_All_t(
                    All_data_np,
                    cluster_labels,
                    init_target,
                    reg=reg
            ...
            print(f"✅ get_All_t 成功，使用 reg = {reg}")
            return pseudotime_z, All_t_z, reg

        except RuntimeWarning as e:
            print(f"⚠️ reg={reg} 出现数值问题，尝试增大 reg")

            reg += reg_step

            if reg > reg_max:
                raise ValueError(
                    f"reg 已增大至 {reg}，仍然数值不稳定，请检查数据或算法"
                ) from e

    raise RuntimeError("超过最大尝试次数，get_All_t 仍然失败")
...
for r in range(epoch):
    ...
    # # Find unique time points t_unique and their corresponding indices
    # t_unique, inverse_indices = torch.unique(t, return_inverse=True)
    # # Compute z_unique
    # z_unique = torch.zeros((len(t_unique), z.size(1)), device=z.device)
    # for j in range(len(t_unique)):
    #     z_unique[j] = z[inverse_indices == j].mean(dim=0)
    #
    # loss_var = z_unique.var(dim=0).mean()
    ...
    if (r+1) % interval == 0:
        with torch.no_grad():
            ...
            pseudotime_z, All_t_z, _ = get_All_t_auto_reg(All_data_np, cluster_labels, init_target, reg_init=1e-1, reg_step=1e-2)
...
import sys, os
os.makedirs('your/path/to/output/glu', exist_ok=True)
os.makedirs('your/path/to/output/glu/gene_dynamic', exist_ok=True)
torch.save(best_model.state_dict(), 'your/path/to/output/glu/model.pth')
# pd.DataFrame(best_t).to_csv('your/path/to/output/glu/best_t.csv')

inn.load_state_dict(torch.load('your/path/to/output/glu/model.pth'))
best_model=inn
z3 = best_model(All_data.float())[0]
z3 = scaled_output(z3)
All_data_np = z3.cpu().detach().numpy()
pseudotime_z, All_t_z = get_All_t(All_data_np, cluster_labels, init_target, reg=2e-1)
All_t = All_t_z
All_t = torch.from_numpy(All_t).to(device)
...
num_inter = 100
name = 'ebdata'
path = 'your/path/to/output/glu/dynamic_Org'
color_use = 'black'
...
with torch.no_grad():
    ...
    plt.close()  # This line ensures the animation finishes before subsequent code runs
...
outputdir = 'your/path/to/output/glu/'
df = inter_plot(All_data.float(), All_t, All_data.float(), cluster_labels, inn, num_inter=100, traj_num=117,  method = 'kde', plot = 'org', reg = 1e-1, outputdir=outputdir)
df = inter_plot(All_data.float(), All_t, All_data.float(), cluster_labels, inn, num_inter=100, traj_num=117,  method = 'kde', plot = 'eu', reg = 1e-1, outputdir=outputdir)
...
source = cluster_labels == 'Normal'
target = cluster_labels == 'T2D'
driver_index = Detect_driver.Detect_driver(All_data.float(), All_t*10, inn, np.ones(117), source=source, target=target, method='kde', reg=2e-1, name=1)
...
driver_genes = gene_names[original_indices[0:100].cpu().detach().numpy()]

pd.DataFrame(dynamic_driver_index_matrix).to_csv(f'{outputdir}/driver_matrix.csv')
